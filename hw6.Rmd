...
library(tidyr)
library(ggplot2)
smallBest <- readxl::read_xlsx('KIB - Best in Show (public).xlsx', sheet = 'Best in show')

fullBest <- readxl::read_xlsx('KIB - Best in Show (public).xlsx', sheet = 'Best in show full sheet')


fullBest <- fullBest[-c(1,2,3),]

fullBestClean <- rename(fullBest, c("Best in show?"="Breed", "largest value" = "Type", "...5"="Score", "...32"="Size", "87"="Pop", "...41"="Intel")) 

fbclean = fullBestClean[,c("Breed","Type","Score","Size","Pop","Intel")] 

fbclean= drop_na(fbclean)

fbclean <- fbclean %>%
  mutate(Score = as.numeric(Score),
         Pop = as.numeric(Pop),
         Type = as.factor(Type),
         Intel = as.factor(Intel),
         Size = factor(Size,levels=c("small","medium","large")),
         IntelBi = ifelse(Intel %in% c("Brightest","Excellent","Above average"), "clever", "dumb"))
...
```{r}
fbclean %>%
  ggplot(aes(x=Score, y=Pop, color = Type, shape = IntelBi)) + 
  geom_point(aes(size = Size)) + 
  geom_text(aes(label = Breed), size = 2,  nudge_y = -3) + 
  geom_vline(xintercept = 2.4) + 
  geom_hline(yintercept = 60) +
  expand_limits(x=c(1,3.8), y=c(0,110)) +
  scale_y_reverse() +
  labs(x = "our data score", y= "popularity", title = "Best In Show: Ultimate Dog Data", size = "Size", color = "Type", shape= "Intellegence") + 
  ...
  annotate("text", x = 1.5, y = 100, label = "Rightfully Ignored" ) +
  annotate("text", x = 3.5, y = 100, label = "Overlooked Treasures" ) 
